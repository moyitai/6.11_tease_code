...
static int norflash_f_erase(int cmd, u32 addr)
{
    u32 type = 0;
...
static int ex_flash_program_loop(program_info_t *info)
{
    ...
    u32 erase_len =  __SECTOR_4K_ALIGN(info->remote_file_length);
    ...
    do {
        ...
        if (info->local_erase_hdl) {
            cur_erase_len = __SECTOR_4K_ALIGN(r_len);
            tmp_addr = (local_addr + remain_len) / 4096 * 4096;
            if (erase_addr != tmp_addr) {
                ...
                flash_erase_by_blcok_n_sector(erase_addr, cur_erase_len, info->local_erase_hdl);
            }
        }
...
static u16 ex_flash_local_file_verify(u8 *buf, u32 addr, u32 len, u32(*read_func)(void *buf, u32 addr, u32 len))
{
    u16 crc_temp = 0;
    u16 r_len;

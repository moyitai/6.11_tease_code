...
void ci_data_rx_handler(u8 type)
{
    ...
    if (type == CI_UART && __this->udev) {
        __this->data_length += __this->udev->read(&__this->pRxBuffer[__this->data_length], (UART_RX_SIZE - __this->data_length), 100);//串口读取buf剩余空间的长度，实际长度比buf长，导致越界改写问题
    }
    ...
    crc16 = CRC16(__this->pRxBuffer, UART_FORMAT_HEAD - 3);

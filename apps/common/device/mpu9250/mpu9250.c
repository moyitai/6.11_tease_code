...
#define MPU9250_INT_IO		TCFG_TP_INT_IO //pg5
...
u8 mpu9250_init1(void *param)//bypass模式读取ak8963数据
{
    ...
    gpio_set_direction(MPU9250_INT_IO, 1);
    gpio_set_die(MPU9250_INT_IO, 1);
    gpio_set_pull_up(MPU9250_INT_IO, 1);
    gpio_set_pull_down(MPU9250_INT_IO, 1);
...
void mpu9250_test2()
{
    ...
    if (mpu9250_init2(&mpu9250_iic_info_test2)) {
        ...
        /* port_wkup_enable(MPU9250_INT_IO, 1, port_wkup_irq_cbfun_test); //PA08-IO中断，1:下降沿触发，回调函数port_wkup_irq_cbfun_test */
#ifdef CONFIG_CPU_BR23
        io_ext_interrupt_init(MPU9250_INT_IO, 1, port_wkup_irq_cbfun_test);
#elif defined(CONFIG_CPU_BR28)
        // br28外部中断回调函数，按照现在的外部中断注册方式
        // io配置在板级，定义在板级头文件，这里只是注册回调函数
        port_edge_wkup_set_callback_by_index(1, port_wkup_irq_cbfun_test); // 序号需要和板级配置中的wk_param对应上

...
struct hw_eq_ch {
    ...
    volatile unsigned int active : 1;    //已启动eq处理  1：busy  0:处理结束
        ...
        unsigned int run_mode: 2;  //0按照输入的数据排布方式 ，输出数据 1:单入多出，  2：立体声入多出
        unsigned int in_mode : 2; //输入数据的位宽 0：short  1:int  2:float
        unsigned int out_32bit : 2; //输出数据的位宽 0：short  1:int  2:float
        unsigned int out_channels: 4; //输出通道数
        unsigned int data_in_mode: 1; //输入数据存放模式
        unsigned int data_out_mode: 1; //输入数据存放模式
#ifdef CONFIG_EQ_NO_USE_COEFF_TABLE
        u32 sr;                          //采样率
#endif

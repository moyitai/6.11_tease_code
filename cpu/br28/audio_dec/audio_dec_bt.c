...
surround_hdl *surround_open_demo(u8 ch_type);
...
#endif
#if AUDIO_CODEC_SUPPORT_SYNC
    audio_way_set_syncts(bt_a2dp_dec->syncts.way_type, NULL);
...
static int a2dp_dec_start(void)
{
...
#if AUDIO_SURROUND_CONFIG
    //环绕音效
    dec->surround = surround_open_demo(ch_type);
#endif
#if AUDIO_VBASS_CONFIG
    //虚拟低音
    ...
    if (AUDIO_DEC_BT_MIXER_EN) {
        a2dp_decoder_syncts_setup(&dec->syncts, fmt->sample_rate, audio_mixer_get_sample_rate(&mixer), ch_num);
    } else {
        a2dp_decoder_syncts_setup(&dec->syncts, fmt->sample_rate, fmt->sample_rate, ch_num);
    }
    /*使能同步，配置延时时间*/
    a2dp_decoder_stream_syncts_enable(&dec->dec, &dec->syncts, fmt->sample_rate, CONFIG_A2DP_DELAY_TIME);
    //sound pcm 设置同步句柄
    dec->syncts.way_type = AUDIO_OUT_WAY_TYPE;
    audio_way_set_syncts(dec->syncts.way_type, &dec->syncts);
...
#if AUDIO_CODEC_SUPPORT_SYNC
    audio_way_set_syncts(dec->syncts.way_type, NULL);
    a2dp_decoder_syncts_free(&dec->syncts);
#endif /*AUDIO_CODEC_SUPPORT_SYNC*/
...
{
    void *limiter = audio_decoder_malloc(need_limiter_noiseGate_buf(1));
...
#if AUDIO_CODEC_SUPPORT_SYNC
    audio_way_set_syncts(bt_esco_dec->syncts.way_type, NULL);
...
static int esco_dec_start()
{
    ...
    memset(&dec->syncts, 0, sizeof(struct decoder_syncts));
    if (AUDIO_DEC_BT_MIXER_EN) {
        esco_decoder_syncts_setup(&dec->syncts, dec->dec.sample_rate, audio_mixer_get_sample_rate(&mixer), dec->dec.ch_num, dec->dec.esco_len);
    } else {
        esco_decoder_syncts_setup(&dec->syncts, dec->dec.sample_rate, dec->dec.sample_rate, dec->dec.ch_num, dec->dec.esco_len);
    }
    /*使能同步，配置延时时间*/

...
typedef enum {
    ...
    EFFECTS_ONLINE_CMD_WRITE_FILE_SIZE = 0x20,  //准备写入文件
    EFFECTS_ONLINE_CMD_WRITE_FILE_CHANNEL = 0x21,
...
void *effects_fopen(const char *path, const char *mode);
int effects_fread(void *file, void *dst, u32 len);
int effects_fpos(void *file);
int effects_flen(void *file);
void effects_fseek(void *file, u16 tar, u16 src);
void effects_fclose(void *file);
